	...
	$("#account_details_modal").on("show.bs.modal", function(e) {
		...
		if (NRS.accountInfo.errorCode && NRS.accountInfo.errorCode != 5) {
			...
			if (NRS.accountInfo.errorCode && NRS.accountInfo.errorCode == 5) {
				...
				$("#account_balance_balance").html(NRS.formatAmount(new BigInteger(NRS.accountInfo.balanceNQT)) + " CARET");
				$("#account_balance_unconfirmed_balance").html(NRS.formatAmount(new BigInteger(NRS.accountInfo.unconfirmedBalanceNQT)) + " CARET");
				$("#account_balance_effective_balance").html(NRS.formatAmount(NRS.accountInfo.effectiveBalanceNXT) + " CARET");
				$("#account_balance_guaranteed_balance").html(NRS.formatAmount(new BigInteger(NRS.accountInfo.guaranteedBalanceNQT)) + " CARET");
